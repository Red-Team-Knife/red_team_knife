...
class SearchsploitController(Controller):
    ...
    def run(self, query):
        ...
        command_thread = SearchsploitCommandThread(None, self)

        command_thread.set_command(command)
        ...
        if self.type_query == EDB:
            ...
            for result in self.last_scan_result["RESULTS_EXPLOIT"]:
                ...
                if result.get("URL", False):
                    ...
                    if result["URL"] == f"https://www.exploit-db.com/exploits/{self.query}":
              
                        html_string += '<p><b>Title: </b>'
                        html_string += f'{result["Title"]}</p><br>'

                        html_string += '<p><b>Url: </b>'
                        html_string += f'<a href= "{result["URL"]}">{result["URL"]}</a></p><br>'    
